...
async def handle_acquire(request):
    ...
    await redis_client.append('semaphore_stats', ujson.dumps(EXEC_STATS))
    await redis_client.aclose()

    return web.Response(text=f'Semaphore acquired by node {NODE}. Success: {acquired}')
...
async def handle_release(request):
    ...
    return web.Response(text=f'Semaphore released by node {NODE}.')
